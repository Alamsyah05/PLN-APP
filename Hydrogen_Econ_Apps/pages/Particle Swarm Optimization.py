...
st.title('PSO Clustering')

# Constants for PSO
number = st.number_input("Masukkan Nilai Beban Kognitif (skala 0 - 1)", min_value=0.00, max_value=1.00, format="%.2f")
numbers = round(number, 2)
cw = numbers
sw1 = 1 - cw
sw = round(sw1, 2)
write = st.write("Nilai Beban Kognitif: ", cw)
write1 = st.write("Nilai Beban Sosial: ", sw)
st.write("")
num_particles = st.number_input("Masukkan Jumlah Partikel", min_value=0)
st.write("Jumlah Partikel: ", num_particles)
st.write("")
iw_initial = st.number_input("Masukkan Beban Inersia Awal", min_value=0.00, value=0.9)
st.write("Beban Inersia Awal: ", iw_initial)
st.write("")
iw_final = st.number_input("Masukkan Beban Inersia Final", min_value=0.00, value=0.1)
st.write("Beban Inersia Final: ", iw_final)
st.write("")

uploaded_files = st.file_uploader("Unggah file Excel berupa data Koordinat SPBU, Koordinat SPKLU, Koordinat PLTH", accept_multiple_files=True, type=['xlsx'])
if uploaded_files is not None:
    ...
    st.warning("Mohon unggah file Excel terlebih dahulu.")
